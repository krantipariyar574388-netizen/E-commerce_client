...
const ProductCard = ({
  products: { _id, name, cover_image, description, rate, category, brand },
}: IProps) => {
  ...
  return (
    <div className="relative h-full max-w-80 border border-gray-300 rounded-md group flex flex-col overflow-clip">
      <button
        ...
        className="absolute top-2 right-1 z-10 cursor-pointer border border-yellow-600 h-8 aspect-square rounded-full flex justify-left items-center p-1 bg-yellow-500/30"
      >
        <FaRegHeart
          className={`size-5 ${inWishlist ? "text-yellow-600 fill-yellow-600" : "text-yellow-600"}`}
        />
      </button>
      {/* image */}
      ...
      <div className="p-1 pb-2 flex flex-col gap-1 mt-2 grow">
        ...
        <div className="flex gap-1 items-center my-1">
          <p className="px-1 py-0.5 border border-yellow-500 bg-yellow-500/20 rounded-lg text-xs">
            {category.name}
          </p>
          <p className="px-1 py-0.5 border border-yellow-500 bg-yellow-500/20 rounded-lg text-xs">
            {brand.name}
